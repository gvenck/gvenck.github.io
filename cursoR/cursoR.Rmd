...
* pelos alunos de pós-graduação Cristiane Taniguti, Fernando Correr, Ana Letycia, Amanda Avelar e Guilherme Hosaka dos laboratórios de Genética Estatística e Bioinformática da ESALQ-USP.
...
Sugerimos que, antes de iniciar a prática aqui descrita, siga [este tutorial](https://gvenck.github.io/events/) para instalação do R e do RStudio.
...
```{r, eval=FALSE}
dados <- read.csv(file = "http://cristianetaniguti.github.io/cursoR/respostas.csv", stringsAsFactors = FALSE, na.strings = "-")
head(dados)
```
...
Para que seja possível imprimir conteúdo de objetos durante o loop, usamos a função `cat`, ela não separa cada resposta em uma linha, precisamos coloca o `\n` indicando a quebra de linha.

Também, se não quisermos trabalhar com dados não padronizados, podemos impedir que o loop continue quando encontrar algum erro. Podemos, para isso, simplesmente quebrá-lo quando encontrar uma determinada condição (nesse caso a falta do "-").
...
for(i in 1:nrow(dados)){
  if(grepl("-", dados[i,5])){
    ...
    stop("Precisamos adicionar mais informações na linha", i, "\n")
  }
}
```

Se não quisermos ser tão exigentes, podemos só dar um aviso.
...
for(i in 1:nrow(dados)){
  if(grepl("-", dados[i,5])){
    ...
    warning("Precisamos adicionar mais informações na linha", i, "\n")
  }
}
```

Ou também podemos só ignorar essas linhas
...
Existem diversas outras ações que podemos usar dentro dos loops conforme nossos interesses.
...
Se diferencia do `apply` por poder receber outros tipos de objetos, mais utilizado com listas, e devolver o resultado também em uma lista.
...
Normalmente é uma boa prática criar um bloco de código se vai realizar aquela ação poucas vezes. Se for realizar várias vezes a ação e ela tiver apenas um objeto como entrada, vale a pena fazer um loop. Mas, se for realizar diversas vezes e o objeto de entrada for modificado vale a pena fazer uma função. E, na hierarquia, quando tiver acumulado muitas funções para realizar uma tarefa mais complexa, vale a pena construir um pacote.
...
dotchart(results$Altura, labels = results$Cidade)
#Também é possível criar grupos
dotchart(results$Altura, labels = results$Cidade, groups = results$Genero)

dados$color[dados$Area=="Exatas"] <- "darkblue"
dados$color[dados$Area=="Biológicas"] <- "darkgreen"
dados$color[dados$Area=="Humanas"] <- "orange"
dados$color[dados$Area=="Interdiciplinar"] <- "red"
dados$color[dados$Area=="Agrárias"] <- "black"
dotchart(dados$Altura, labels = dados$Cidade, groups = dados$Area, color = dados$color)
...
barplot(t(nota_gen), beside = TRUE, col = c("wheat", "tomato2"), 
        main = "Distribuição das pessoas \n de acordo com área e nota do R",
        xlab = "Nota do R")
legend("topright", title = "Gênero", legend=c("F","M"),
       fill = c("wheat", "tomato2"), bty = "n", cex = 0.7)
...
dotchart(dados$Peso, labels = dados$Cidade, groups = dados$Area, color = dados$color,
         font=2,
         main = "Peso de acordo com as cidades, \n agrupando por área",
         xlab = "Peso", pch = 7)
par(mfrow=c(1,1))
...
Verifique que, algumas coordenadas não puderam ser encontradas, será necessário corrigir o nome das cidades e estados para essas.

```{r, warning=FALSE, message='hide', results='hide', eval=FALSE}

pos.na <- which(is.na(coord[,1]))

dados$Cidade[pos.na]

correcao <- c("Piracicaba,SP", "Rio das Pedras,SP")

coord.corr <- data.frame()
for(i in 1:length(correcao)){
  coord.corr <- rbind(coord.corr,geocode(correcao[i]))
}

coord.corr

coord[pos.na,] <- coord.corr
